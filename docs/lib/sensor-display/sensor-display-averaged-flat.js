...
function sensorReading(name, type, units, reading, readingTaken, valueClasslist) {
  ...
  value.classList.add("value-badge-outline", "is-size-4");
  var valueP = document.createElement("P");
  valueP.innerText = '' + reading + units;
  value.appendChild(valueP);

  var labelDiv = getInfoIconLinkWithText(GetPollutionBand(reading), "element-toggle");
...
function GetPollutionBand(reading) {
  return "Low";
}
